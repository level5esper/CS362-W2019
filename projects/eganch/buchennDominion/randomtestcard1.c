...
void checkSmithy(struct TestState *testState, struct gameState *post, int handPos) {
  ...
  if (!randomAssertTrueForIntComparison(testState, originalHandNum + 2, post->handCount[playerNum]) && TEST_DEBUG) {
    printf("X - Hand count incorrect\n");
  }

  //deck size has decreased by three
  if (!randomAssertTrueForIntComparison(testState, originalDeckCount - 3, post->deckCount[playerNum]) && TEST_DEBUG) {
    printf("X - Deck size incorrect\n");
  }

  //smithy should have been added to played cards
  if (!randomAssertTrueForArrayHoldingCardValue(testState, post->playedCards, post->playedCardCount, smithy) && TEST_DEBUG) {
...
int main() {
  ...
  for (int n = 0; n < 2000; n++) {
    int handPos = setUpRandomGameState(&gameState, smithy);
