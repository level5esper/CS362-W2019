...
void checkAdventurer(struct TestState *testState, struct gameState *post) {
  ...
  if (!randomAssertTrueForIntComparison(testState, originalHandNum + 1, post->handCount[playerNum]) && TEST_DEBUG) {
    printf("X - Hand count incorrect\n");
  }

  //deck size has decreased
  if (!randomAssertTrueForGreaterThan(testState, originalDeckCount, post->deckCount[playerNum]) && TEST_DEBUG) {
    printf("X - Deck size incorrect\n");
  }

  //adventurer should have been added to played cards
  if (!randomAssertTrueForArrayHoldingCardValue(testState, post->playedCards, post->playedCardCount, adventurer) && TEST_DEBUG) {
...
int main() {
  ...
  for (int n = 0; n < 2000; n++) {
    setUpRandomGameState(&gameState, adventurer);
